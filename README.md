...
## Experiment 5 conservative tuning

A reproducible 36-candidate `RandomizedSearchCV` used five shuffled stratified seed-42 folds to tune the unchanged gradient-boosting pipeline. The best sampled configuration (`n_estimators=60`, `learning_rate=0.05`, `max_depth=4`, `min_samples_split=2`, `min_samples_leaf=8`, `subsample=0.85`, `max_features=None`) scored 0.8473 on the search folds versus 0.8395 for the default model on identical folds.

That apparent +0.0078 gain did not survive the separate 20-seed × 5-fold confirmation. The tuned model averaged 0.8304 ± 0.0257 across all folds versus 0.8325 ± 0.0250 for the default. Its mean paired seed difference was -0.0021 (median -0.0023), it won 6 seeds and lost 14, and its paired seed-bootstrap 95% interval was [-0.0048, +0.0006]. This is evidence of selection sensitivity rather than a robust improvement. The default GradientBoosting model remains champion, no Kaggle submission is recommended for the tuned candidate, and no new submission file was created. See `notebooks/05_gradient_boosting_tuning.ipynb` for the search space, top candidates, full confirmation, visualization, and complexity discussion.
